...
bool haveNumber(const std::vector<int> & v, Iter left, Iter right, const int & target);
...
int main(void)
{
    ...
    while (std::cin >> number_to_find)
        std::cout << (haveNumber(numbers, numbers.begin(),
            numbers.end(), number_to_find) ? "Have" : "Don't have")
        << " " << number_to_find << '\n'
